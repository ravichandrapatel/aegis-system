# 15. Optional future: multi-agent split of `AGENTS.md`
...
This page is the human operator guide for the *optional* enhancement: exposing OKF as multiple IDE agents while keeping **one brain**.
...
| IDE UX | Users cannot pick “OKF Maintain” vs “OKF Generate” explicitly |
...
| Split before pain is proven | Violates Simplicity First (Laziness Ladder) |
...
An agent file gates **pipeline shape** only. There is no second capability-template dimension: the package ships no Profile registry, and the only capability signal is the `caps:` line from `okf.py pack` / `capabilities`.
...
- [ ] Shared Rule #1 (Pack First) + Prompt Card rules  
- [ ] Shared knowledge precedence + evidence grades (agent conventions, not kernel-enforced)  
- [ ] Shared pack budget (≤ 8 cards; ~1200 token budget — kernel defaults)  
- [ ] MAINTAIN still binds to `maintain-okf-system.md`  
- [ ] Capability discovery stays the shared `okf.py pack` / `capabilities` probe — `BLOCKED` means stop, not a per-agent allow-list  
...
| --- | --- |
| Use the intent matrix in one `AGENTS.md` | Invent parallel protocols |
...
See also: [Protocol & routing](06-protocol-routing.md), [Pipelines](11-pipelines-and-outputs.md).
