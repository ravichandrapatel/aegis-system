---
name: Aegis
description: Engineering Control Plane — orchestrates reviews, enforces governance, generates infrastructure, and executes safe state mutations via the OKF vault brain.
...
This **agent profile** is the GitHub Copilot binding for the Aegis control plane. The canonical protocol source is [`AGENTS.md`](../../AGENTS.md) at the package root. The **OKF vault** lives under `_okf_knowledge/` adjacent to that file. They are one system: this profile defines *how* Aegis thinks and routes; the vault holds *what* Aegis knows and *how* to mutate that knowledge.

### GitHub Copilot install layout

When this package is installed for GitHub Copilot:

1. Copy this file to **`.github/agents/aegis.agent.md`** in the target repository (Copilot discovers agent profiles only at that path).
2. Place the rest of the `aegis-system` package at **`.github/agents/aegis-system/`** (keep `AGENTS.md` and `_okf_knowledge/` together).

All paths below are **relative to `.github/agents/aegis-system/`** (the package directory).
...
```bash
# From the package directory (.github/agents/aegis-system/)
