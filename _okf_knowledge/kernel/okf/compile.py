...
from okf.textutil import norm as _norm, tokenize as _tokenize
from okf.trust import trust_tier
...
def compile_graph(concepts: list[Concept]) -> dict[str, list[dict[str, str]]]:
    ...
    intent: Build the node/edge graph implied by markdown cross-links. Nodes
            carry the concept's markdown body so okf-brain's reading pane
...
def _index_row_for_concept(c: Concept) -> dict[str, object]:
    ...
    return {
        ...
        "tags": tag_strs,
        "type": ctype,
        "resource": str(c.frontmatter.get("resource", "") or ""),
        "trust": trust_tier(c.frontmatter),
        "stale_after": str(c.frontmatter.get("stale_after", "") or ""),
        "id_norm": id_norm,
...
def _artifacts_fresh(concept_count: int, card_count: int) -> bool:
    """
    intent: True only when every compile artifact exists, parses, and agrees
            with the current vault.
    input: expected concept count; expected Prompt Card count.
    output: bool — False forces a full rewrite.
    role: incremental-compile guard.
    side_effects: reads the three artifacts.

    Checking index.json alone was not enough: a deleted or corrupted
    prompt_cards.json / graph.json could survive a compile, leaving lookup
    serving cards that no longer matched the vault.
    """
    ...
    try:
        ...
        if not (isinstance(raw, dict) and isinstance(raw.get("entries"), list)):
            return False  # v1 list form → force upgrade to v2
        if len(raw["entries"]) != concept_count or raw.get("version") != INDEX_FORMAT_VERSION:
            return False
        cards = json.loads(cards_p.read_text(encoding="utf-8"))
        if not isinstance(cards, dict) or len(cards) != card_count:
            return False
        graph = json.loads(GRAPH_JSON.read_text(encoding="utf-8"))
        if not isinstance(graph, dict) or not isinstance(graph.get("nodes"), list):
            return False
        if len(graph["nodes"]) != concept_count:
            return False
    except (OSError, json.JSONDecodeError):
        return False
    return True
...
def cmd_compile(args: object | None = None) -> int:
    ...
    intent: Write graph.json, index.json (v2+inverted), prompt_cards.json;
            embed graph into okf-brain.html. Skips work when the incremental
    ...
    try:
        ...
        expected_cards = sum(1 for c in concepts if extract_prompt_card(c.body))
        if not force and dirty == 0 and _artifacts_fresh(len(concepts), expected_cards):
            print(
        ...
        # Drop legacy TOON index if present from older OKF versions.
        ...
        for slot in (INDEX_CACHE, ADJ_CACHE, CARD_CACHE, INVERTED_CACHE):
            slot.mtime_ns = None
            slot.payload = None
        embedded = inject_into_brain("graph-data", graph_json)
    except OSError as exc:
    ...
    print(
        ...
        + ("; embedded graph into okf-brain.html" if embedded else "")
    )
    return 0
