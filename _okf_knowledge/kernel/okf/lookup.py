...
from okf.models import Hit, IndexEntry, _MtimeCache
...
from okf.vault import concept_id_from_path, load_vault
...
# Public aliases for compile cache busting / pack (stable API).
...
def _ensure_norms(entry: IndexEntry) -> None:
    ...
    if not entry.title_tokens:
        entry.title_tokens = frozenset(_tokenize(entry.title))
    if not entry.id_tokens:
        entry.id_tokens = frozenset(_tokenize(entry.concept_id))
...
def score_entry(entry: IndexEntry, terms: list[str]) -> tuple[int, list[str]]:
    ...
    # Merge camelCase subword tokens into their OWN field bag only. Merging the
    # combined search_tokens here made a description-only word score as an exact
    # title and id hit (e.g. "how" scoring 83 against an unrelated concept).
    hay_tok["title"] |= set(entry.title_tokens)
    hay_tok["id"] |= set(entry.id_tokens)
    weights = {
        "id": ID_WEIGHT,
        "title": TITLE_WEIGHT,
        "tags": TAG_WEIGHT,
...
# Stable public API for pack / external callers.
...
load_adjacency = _load_adjacency
