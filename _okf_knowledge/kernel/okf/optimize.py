...
from okf.paths import VAULT_ROOT
from okf.vault import format_frontmatter, load_concept
...
def rebuild_references_index() -> int:
    ...
    for source_dir in _reference_cache_dirs():
        ...
        for ref in sorted(source_dir.glob("*.md")):
            ...
            resource = str(c.frontmatter.get("resource", "")).strip()
            official = f" — [official docs]({resource})" if resource else ""
