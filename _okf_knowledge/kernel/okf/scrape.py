...
from okf.config import load_config, scan_secrets
from okf.paths import SKIP_DIRS, VAULT_ROOT
from okf.vault import format_frontmatter
...
def fetch_page(url: str) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "okf-scraper/0.1"})
...
def compress_reference_body(content: str, max_chars: int) -> tuple[str, str]:
    ...
    for line in lines:
        ...
        if under < 3:
            ...
            under += 1
    text_out = "\n".join(out)
    note = "\n\n*(compressed — see resource for full page)*"
...
def write_reference(
    ...
    if leaks:
        raise ValueError(
            ...
            "Redact upstream content or set credential_scan=False in okf.config.DEFAULT_CONFIG."
        )
    cfg = load_config()
    ...
    if cfg.get("reference_compress", True) and len(content) > max_chars:
        ...
        note = (
            "\n\n*(truncated — see resource for the full page)*"
            if len(content) > max_chars
            else ""
        )
    safe_title = re.sub(r"\s+", " ", title).strip()
    source_id = f"{domain_slug}-{safe_slug}"
    fm = {
        "type": "Reference",
        "title": safe_title,
        "description": "Cached upstream documentation, fetched by `okf.py scrape`.",
        "tags": [domain_slug, "upstream", "cached"],
        # OKF reserves `resource` for the pointer to the described thing.
        "resource": url,
        "status": "stable",
        # §5.2: a scrape is machine-generated and nobody has confirmed it, so
        # `verified` is deliberately absent — the concept reads as unverified.
        "generated": {"by": "okf-agent/scrape", "at": now},
        # Provenance lives in frontmatter `sources`; body citation lists are obsolete.
        "sources": [
            {
                "id": source_id,
                "resource": url,
    ...
    doc = (
        ...
        # §5.1 per-claim attribution: a footnote label keyed to a sources[].id.
        + f"Cached verbatim from the upstream page.[^{source_id}]\n\n"
        + f"[^{source_id}]: {safe_title}\n"
    )
...
def cmd_scrape(args: argparse.Namespace) -> int:
    ...
    print("next: python3 _okf_knowledge/kernel/okf.py optimize  (normalize + recompile index)")
    return 0
