...
def load_config() -> dict[str, object]:
    ...
    output: shallow copy of DEFAULT_CONFIG.
    """
...
def load_ignore_patterns() -> list[str]:
    ...
    patterns = _read_ignore_file(VAULT_ROOT / IGNORE_FILE_NAME)
    cfg = load_config()
...
def scan_secrets(text: str) -> list[str]:
    ...
    if not text or not load_config().get("credential_scan", True):
        return []
    found: list[str] = []
