...
class IndexEntry:
    ...
    ctype: str = ""
    # OKF reserved field: pointer to the thing this concept describes
    # (upstream URL, console link, repo path). Not searched — carried so a
    # Reference card can cite its own source.
    resource: str = ""
    # OKF trust signals. `trust` is derived at compile time from `verified`
    # (static content); `stale_after` is kept raw because staleness depends on
    # when the pack is built, not when it was compiled.
    trust: str = ""
    stale_after: str = ""
    path: Path | None = None
    ...
    # Per-field subword tokens. Kept separate from search_tokens so a word that
    # only appears in the description cannot score as an exact title/id hit.
    title_tokens: frozenset[str] = field(default_factory=frozenset)
    id_tokens: frozenset[str] = field(default_factory=frozenset)
